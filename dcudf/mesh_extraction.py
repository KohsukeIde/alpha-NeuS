...
class dcudf:
    """DCUDF mesh extraction
        ...
        """
    def __init__(self,query_func_extract,resolution,threshold,query_func_optim=None,
        ...
        self.query_func_extract = query_func_extract
        self.query_func_optim = query_func_optim if query_func_optim is not None else query_func_extract

    def optimize(self):
        query_func = self.query_func_optim
        ...
        for xi, xs in enumerate(X):
            for yi, ys in enumerate(Y):
                for zi, zs in enumerate(Z):
                    ...
                    val = self.query_func_extract(pts).reshape(len(xs), len(ys), len(zs)).detach().cpu().numpy()
