...
class Dataset:
        ...
        for i, (img_path, mask_path) in enumerate(zip(self.images_lis, self.masks_lis)):
            try:
                ...
                if os.path.exists(img_path) and os.path.exists(mask_path):
                    ...
                    # 有効な画像データをGPUに保持
                    self.images.append(torch.from_numpy(img.astype(np.float32) / 256.0).to(self.device))
                    self.masks.append(torch.from_numpy(msk.astype(np.float32) / 256.0).to(self.device))
                ...
                self.bad_image_indices.add(i)
                logging.error(f"Error loading image at index {i}: {e}")
    ...
    def drop_images_and_masks(self):
        """メモリ節約のために画像データをメモリから削除する"""
        if not self.is_images_dropped:
            self.images = []
            self.masks = []
            self.is_images_dropped = True
            gc.collect()
            logging.info("Dropped images and masks from memory")
    ...
    def gen_random_rays_at(self, img_idx, batch_size):
        ...
        if self.is_images_dropped:
            logging.warning("Images were dropped, cannot access them. Please load the dataset again.")
            # 画像のダミーデータで代用
            ...
            mask_value = mask[(pixels_y, pixels_x)][:, :1]  # batch_size, 1
        ...
        if self.is_images_dropped:
            ...
            try:
                ...
                if img is None:
                    ...
                    if not valid_indices:
                        # 黒い画像を返す
                        return np.zeros((self.H // resolution_level, self.W // resolution_level, 3), dtype=np.uint8)
            ...
            return sample_image_as_numpy(self.images[idx], resolution_level) * 256.0