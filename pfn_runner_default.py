...
# from models.dataset_pfn import Dataset
from models.dataset_lru_default import Dataset
...
class Runner:
    ...
    def load_checkpoint(self, checkpoint_name):
        # weights_only=Falseを明示的に指定して古いチェックポイントを読み込めるようにする
        checkpoint = torch.load(os.path.join(self.base_exp_dir, 'checkpoints', checkpoint_name), 
                            map_location=self.device, 
                            weights_only=False)  # この行を変更
        ...
        for i in range(img_fine.shape[-1]):
            if len(out_rgb_fine) > 0:
                cv.imwrite(os.path.join(self.base_exp_dir,
                           ...
                           np.concatenate([img_fine[..., i], gt_img]))
            if len(out_normal_fine) > 0:
                cv.imwrite(os.path.join(self.base_exp_dir,
                                        'normals',
                                        '{:0>8d}_{}_{}.png'.format(self.iter_step, i, idx)),
